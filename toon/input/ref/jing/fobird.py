...
# from cgkit
# updated to Ivrylab specifications, October 2007 by John Schlerf
...
class BirdError(Exception):
    ...
    This exception is thrown when the Birds do not appear to be running.
    """
    pass
...
class BirdContext:
    ...
    def __init__(self):
        ...
        # 36 for normal range transmitter?  This is what's used in DOS, at least.
        # 2.54 to convert inches into cm
        self.pos_scale = 36.0*2.54
    ...
    def decode(self, s, nWords=-1):
        ...
        for i in range(nWords):
            ...
            v*=self.scales[i]
            v=v/32768.0 # give V in cm (as DOS does)
            values.append(v)
...
#                s += "not dependent\n"
    ...
    def open(self, port=0, baudrate=115200, timeout=3):
        ...
        ser = self.ser
        self.groupOn = False
        self.flockMode = POSITION_ANGLES
...
#        self.ser.setRTS(0)
    ...
    def groupSetup(self, mode='POSITION'):
        # set up 2 birds, returning positions, in group mode
        self.open(timeout=0.2)
        self.fbbAutoConfig(2)
        time.sleep(0.2)
        self.hemisphere(UPPER,1)
        self.hemisphere(UPPER,2)
        if (mode.upper == 'POSITION_ANGLES'):
            self.position_angles(1)
            self.position_angles(2)
            self.flockMode = POSITION_ANGLES
        else:
            self.position(1)
            self.position(2)
            self.flockMode = POSITION
        time.sleep(0.2)
        self.sleep()
        time.sleep(0.1)
        self.SetSampleRate(138)
        time.sleep(0.2)
        self.GroupMode()
        samplerate = self.examineValue(BIRD_MEASUREMENT_RATE)
        #try:
        #    samplerate = self.examineValue(BIRD_MEASUREMENT_RATE)
        #except DataError:
        #    raise BirdError, "Flock does not appear to be flying.  Please try again."
        print "Sampling at " + str(samplerate) + " Hz."
        print "Flock ready and in standby mode."
    ...
    def examineValue(self, param, addr=None):
        ...
        # GROUP MODE SETTING
        elif param==GROUP_MODE:
            v = self._read(1, exc=True)
            res = struct.unpack("<H", v)[0]
        else:
    ...
    def GroupMode(self):
        """toggle Group Mode.
        """
        if self.groupOn==False:
            self._write(CHANGE_VALUE+GROUP_MODE+chr(1))
            self.groupOn = True
            print "Flock in Group Mode"
        else:
            self._write(CHANGE_VALUE+GROUP_MODE+chr(0))
            self.groupOn = False
            print "Flock no longer in group mode"
    ...
    def position_matrix(self, addr=None):
        ...
        ctx.position_matrix()
        if self.groupOn == True:
            self.flockMode = POSITION_MATRIX
    ...
    def position_quaternion(self, addr=None):
        ...
        if self.groupOn == True:
            self.flockMode = POSITION_QUATERNION        

    def point(self, addr=None, returnRecord = False):
        """POINT command.
        ...
        if self.groupOn==False:
            ...
            if len(record)!=ctx.record_size:
                ...
                return None
    #        hexdump(res)
            # JUST ADDED THE ,3 BELOW:
            res = ctx.decode(record,3)
    #        print res
        else:
            self._write(POINT)
            while 1: # catch junk sent from flock?
                record = self._read() # read 1 byte
                if record=="":
                    print "FOB: No feedback from the ERC"
                    return
                if ord(record[0])&0x80:
                    break
                print "FOB: Bit 7 not set?" # uninformative error message, anyone?

            if self.flockMode == POSITION:
                record += self._read(13)
            elif self.flockMode == POSITION_ANGLES:
                record += self._read(25)
            else:
                print "Unanticipated record mode; will be SLOW"
                record += self._readAll()
            # In group mode, with 2 birds there's 1 extra byte/bird.
            # Check for those two bits, and decode without them.
            bird2byte = record[-1]
            if bird2byte != chr(2):
                print "Bird 2 bit improperly set"
                #bird2record = []
                bird2record = record[len(record)/2:-1] # debugging; remove eventually
            else:
                bird2record = record[len(record)/2:-1]
            bird1byte = record[len(record)/2-1]
            if bird1byte != chr(1):
                print "Bird 1 bit improperly set"
                #bird1record = []
                bird1record = record[:len(record)/2-1] # debugging; remove eventually
            else:
                bird1record = record[:len(record)/2-1]
            res = [self.defaultctx.decode(bird1record,3),self.defaultctx.decode(bird2record,3)]
        if returnRecord:
            return record
        else:
            return res
...
##            return v-0x10000 
    ...
    def Sample(self):
        # Going to be lazy and assume group mode
        if self.isStreaming==False:
            res = self.point()
        else:
            # search for phasing bit
            count = 0
            while 1:
                record = self._read() # read 1 byte
                if record=="":
                    count += 1
                    if count > 15:
                        print "Attempted to read 16 times without success..."
                        return None
                elif ord(record[0])&0x80:
                    break
            ...
            if self.flockMode == POSITION:
                ...
                print "Unanticipated record mode; may return junk"
                record += self._read(25)
            # In group mode, with 2 birds there's 1 extra byte/bird.
            # Check for those two bits, and decode without them.
            bird2byte = record[-1]
            ...
            if bird1byte != chr(1):
                ...
                bird1record = record[:len(record)/2-1]
            res = [self.defaultctx.decode(bird1record,numWords),self.defaultctx.decode(bird2record,numWords)]
        return res
    ...
    def SetSampleRate(self,newRate):
        newRate = int(round(newRate))
        if newRate < 80:
            print "Measurement rate too low, use 80."
        elif newRate > 140:
            print "Measurement rate too high, use 140."
        else:
            self._write(CHANGE_VALUE+BIRD_MEASUREMENT_RATE+chr(255)+chr(newRate))

    def StopStream(self):
        self._write(POINT)
        self.isStreaming = False
        # Do some cleanup of the serial port?
        junk = self._readAll()
    ...
    def checkStreamTiming(self,numIter=256):
        self.run()
        time.sleep(1)
        self.StartStream()
        collectedTimes = []
        startTime = time.time()
        # Collect samples, then stop the stream
        for sample in range(numIter):
            junk = self.Sample()
            collectedTimes.append(time.time()-startTime)
        self.StopStream()
        timediffSum = 0.0
        for index in range(1,numIter):
            diff = round(1000*(collectedTimes[index]-collectedTimes[index-1]))
            if diff > 1:
                timediffSum += diff
        timediffMean = timediffSum/numIter
        print "Samples collected:"
        print numIter
        print "Mean interval between, in milliseconds:"
        print timediffMean
        self.sleep()
        junk = self._readAll
